...
contract ImmutableRatings is Ownable2Step, ReentrancyGuard {
    ...
    function createUpRating(string calldata url, uint256 amount) external payable nonReentrant notPaused {
        ...
        _processPayment(amount, paymentToken);
    }
    ...
    /**
     * @notice Gets the address of a URL from the Immutable Mapping contract or creates a new one if it doesn't exist
     * @param url The url of the market
     * @param from The address of the user
    ...
    /**
     * @notice Gets the price based on the number of ratings being created
     * @param amount The amount of tokens to rate
     ...
     * @param amount The amount of tokens to rate
     */
    function _processPayment(uint256 amount, address token) internal {
    ...
    function _distributePaymentErc20(uint256 amount, address from) internal {
        if (from == address(this)) {
            SafeERC20.safeTransfer(IERC20(paymentToken), receiver, amount);
        } else {
            SafeERC20.safeTransferFrom(IERC20(paymentToken), from, receiver, amount);
        }
    }
